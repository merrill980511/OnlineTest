...
<html>
<head>
    <title>试卷添加问题页面</title>
</head>
<body>
<form action="/admin/testPaper?cmd=addQuestion" method="post">
    <input type="hidden" name="testPaperId" value="${queryQuestion.id}"/>
    <a>自动生成试卷</a>
    <center>
            ...
            <tr>
                ...
                <td align="center">
                    ...
                    <c:forEach var="option" items="${question.options}">
                        <textarea cols=20 rows=1 readonly>${option.item}.${option.content}</textarea>
                    </c:forEach>
                </td>
            </tr>
        </c:forEach>
    ...
    function add() {
        ...
        if(!flag){
            ...
            if(confirm("确定要执行此操作吗?")){
                document.forms[0].action = "/admin/testPaper?cmd=add";
                document.forms[0].submit();
            }
        }
    }
